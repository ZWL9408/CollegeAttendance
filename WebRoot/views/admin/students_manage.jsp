...
$(".panel3").toggle();
  ...
  <jsp:include page="/views/head.jsp"/>
		<div class="container">
			<div class="header"><h1>学生基本信息如下：</h1></div>
	  	...
	  	<table class="table4_2" align="center" width="98%" border="100" cellspacing="1" style="border:1pt dashed; font-size:15pt;" height="31">
	  	<tr>
	  		<td>学号</td>
	  		<td>姓名</td>
	  		<td>性别 </td>
	  	...
	  	<%
	  		...
	  		List<students> substudents=(List<students>)request.getAttribute("substudents");
	  		if(!substudents.isEmpty()){
	  			for(int i=0;i<substudents.size();i++){
	  			 ...
	  			 out.print("<td>"+Students.getGender()+"</td>");
	  			 out.print("<td>"+Students.getClazz()+"</td>");
	  			 out.print("<td>"+Students.getPassword()+"</td>");
	  	 %>
	  	 <td>
	  	 	<a class="button red" href="studentsServlet?student_id=<%=Students.getStudent_id()%>&Students=<%=2%>"
	  	 	 onclick="return confirmdialog()"> 删除</a>
	  	 	<a class="button blue" href="studentsServlet?student_id=<%=Students.getStudent_id()%>&Students=<%=4%>">
	  	 	修改</a>
	  	 </td>
	  	 <%
	  	 	out.print("</tr>");
			...
			if (last < 0) {
				...
				out.print("<a href='studentsServlet?currentRecord=" + last
						+ "&Students=1'>上一页</a>|");
			}
			if (next >= pager.getTotalRecord()) {
				out.println("尾页|");
			} else {
				out.print("<a href='studentsServlet?currentRecord=" + next
						+ "&Students=1'>下一页</a>|");
			}
		%> 
	  </font></span></center>
