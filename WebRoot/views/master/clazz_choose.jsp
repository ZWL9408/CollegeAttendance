...
$(".panel2").toggle();
  ...
  <jsp:include page="/views/head.jsp"/>
		<div class="container">
			<div class="header"><h1>选择班级进行考勤</h1></div>
			...
			<div style="height:20px;  float:right"></div>
			  <table class="table4_2" align="center" width="100%" border="100" cellSpacing=1 style="border:1pt;font-size:15pt;"height="31">
			    ...
			    <%
						...
						List<course> subResult = (List<course>) request.getAttribute("fuck");
						if (!subResult.isEmpty()) {
							for (int i = 0; i < subResult.size(); i++) {
							     course Absence=subResult.get(i);
					  			 out.print("<tr>");
					  			 out.print("<td>"+Absence.getCourse_name()+"</td>");
					  			 out.print("<td>"+Absence.getName()+"</td>");
					  			 out.print("<td>"+Absence.getClazz()+"</td>");%>
					  	<td><a class="button pink" href="clazzServlet?clazz=<%=Absence.getClazz()%>&clazz_method=<%=1%>">
					  		查看该班</a>
					  		<a class="button pink" href="co_timeServlet?course_name=<%=Absence.getCourse_name()%>&co_time=<%=1%>">
					  		选择考勤时间</a>
					  	</td>		 
