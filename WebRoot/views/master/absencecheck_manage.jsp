...
$(".panel2").toggle();
  ...
  <jsp:include page="/views/head.jsp"/>
		<div class="container">
			<div class="header"><h1>待审核学生请假申请</h1></div>
			...
			<div style="height:20px;  float:right" ></div><br>
				<table class="table4_2" align="center" width="90%" border="1" cellspacing="1" style="border: 1pt dashed; font-size: 15pt;" height="31px">
					...
					<%  response.setCharacterEncoding("UTF-8");
						...
						List<absence> subResult = (List<absence>) request.getAttribute("subcheckResult");
					...
					%>
					<td><a class="button pink" href="absencecheckServlet?student_id=<%=Absence.getStudent_id()%>&methodName=<%=2%>"
						onclick="return confirmdialog()">删除</a>
						<a class="button blue" href="absencecheckServlet?student_id=<%=Absence.getStudent_id()%>&methodName=<%=4%>">审核</a>
					</td>
					<% out.print("</tr>");}}%>
				</table>
				<br>
			...
			<span style="width:100%; text-align:center; display:block;"><font size="3pt">
				...
				<%  int last = pager.getCurrentRecord() - pager.getPageSize();
					...
					if (last < 0) {
						...
						out.print("<a href=''absencecheckServlet?currentRecord="+last+"&methodName=1'>上一页</a>|");
					}
					if (next >= pager.getTotalRecord()) {
						out.println("尾页|");
					} else {
						out.print("<a href='absencecheckServlet?currentRecord="+next+"&methodName=1'>下一页</a>|");
					}
				%> </font> </span>
			</div>
		</div>
